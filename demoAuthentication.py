...
def verify_login(username, password, role):
    try:
        ...
        cursor = conn.cursor()
        query = "SELECT * FROM users WHERE user_name = %s AND password = %s AND role = %s"
        # print(f"Executing query: {query} with values: {username.strip()}, {password.strip()}, {role}")
        cursor.execute(query, (username.strip(), password.strip(), role))
        user = cursor.fetchone()
        # print(f"User fetched: {user}")  # Debugging print statement
        if user:
            cursor.execute("UPDATE users SET logged_in = %s WHERE user_name = %s", (True, username.strip()))
        conn.commit()
        cursor.close()
        conn.close()
...
def log_out(username):
    try:
        conn = psycopg2.connect(**conn_details)
        cursor = conn.cursor()
        cursor.execute("UPDATE users SET logged_in = %s WHERE user_name = %s", (False, username.strip()))
        conn.commit()
        cursor.close()
        conn.close()
...
def sign_in():
    ...
    # print(f"Username: {username}, Password: {password}, Role: {role}")  # Debugging print statement

    if verify_login(username, hash_password(password), role):
        if role == "admin":
...
#         conn = psycopg2.connect(**conn_details)
#         cursor = conn.cursor()
#         cursor.execute("UPDATE users SET logged_in = %s WHERE user_name = %s", (False, username))
#         conn.commit()
#         cursor.close()
#         conn.close()
#         messagebox.showinfo("Logged Out", "You have successfully logged out.")
