...
import csv
import hashlib
from data import DataVisualisation
...
def search_product(bar_code):
    ...
    try:
        ...
        if results:
            result_str = "\n".join([f"Barcode: {row[0]}\n Name: {row[1]}\n Quantity: {row[2]}\n Category: {row[3]}\n Price: {row[4]}" for row in results])
...
def search_product_window():
    ...
    ctk.CTkLabel(modify_window, text="Enter barcode").grid(row=0, column=0, padx=20, pady=10)
    bar_code_entry = ctk.CTkEntry(modify_window)
    ...
    ctk.CTkButton(modify_window, text="Search", command=submit).grid(row=4, column=0, columnspan=2, padx=20, pady=20)
...
class ManegerPage(ctk.CTk):
    def __init__(self, username):
        ...
        load_BGImg(self, 'assets/adminBG.png', 'assets/adminBG.png')
        ...
        self.btn_data_and_statistics = ctk.CTkButton(self.nav_frame, text="Data and Statistics", 
                                                command=self.initialize_data_visualisation, 
                                                width=150, height=50)
    ...
    def initialize_data_visualisation(self):
        # Initialize the DataVisualisation class
            DataVisualisation()
    ...
    def show_add_product(self):
        ...
        ctk.CTkLabel(self.content_frame, text="Category:").grid(row=5, column=0, sticky="e", padx=20, pady=5)
        category_entry = ctk.CTkEntry(self.content_frame)
    ...
    def show_view_products(self):
        ...
        try:
            ...
            treeview = ttk.Treeview(self.content_frame,columns=("Barcode", "Name" ,"Price","Category","Quantity"), show="headings")
            treeview.column("Barcode", width=120, anchor='center')  # Adjust the width as needed
            treeview.column("Name", width=120, anchor='center')     # Adjust the width as needed
            treeview.column("Price", width=120, anchor='center')    # Adjust the width as needed
            treeview.column("Category", width=120, anchor='center')
        ...
        ctk.CTkButton(self.content_frame, text="Search Product", command=search).grid(row=4, column=0, padx=20, pady=10)
        ctk.CTkButton(self.content_frame, text="Export to CSV", command=export,fg_color="green").grid(row=5, column=0, padx=20, pady=10)
        ctk.CTkButton(self.content_frame, text="Delete Product", command=delete, fg_color="red").grid(row=6, column=0, padx=20, pady=10)
...
def load_BGImg(app, light_image_path, dark_image_path):
    try:
        ...
        dark_image = Image.open(dark_image_path).resize((300, 860))
        my_image = ctk.CTkImage(light_image=light_image, dark_image=dark_image, size=(300, 860))
